...
class PowerModel extends BleModel {
  ...
  void startScan() async{
    if (await checkPermission()) {
      super.startScan();
      int _flag = 0;
      BleManager.instance.startScan([Guid(BlueUUID.FS_UUID), Guid(BlueUUID.FTMS_UUID)], [], [RHDeviceType.powerBoard], (result) {
        int newFlag = ++_flag;
  ...
  sortScanResult(List<ScanResult> result) async {
     List<RHBlueScanResult> resultList = await _repository.sortScanResult(result, [RHDeviceType.powerBoard, RHDeviceType.powerAdvanced]);
  ...
  setDeviceConfig({
    ...
    sendCmd(PowerCommands.setDeviceParams(
  ...
  getDeviceConfig() {
    sendCmd(PowerCommands.getDeviceParams());
  }
