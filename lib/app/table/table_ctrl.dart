...
class TableCtrl extends GetxController {
  RxInt flag = 0.obs;
  ...
  void onInit() {
    ...
    subscription = BleManager.instance.scanningStreamController.stream.listen((isScanning) {
      if (isScanning) {
    ...
    stateSubscription = powerModel.bleDeviceStateController.stream.listen((msg) {
      switch (msg) {
      //一切都准备好了
        ...
        case BleDeviceStateMsg.deviceCheckSuccess: {
          LogUtils.d("进入详情界面： ");
          Get.toNamed(RHRoute.tableDetail);
        }
        default:
          break;
      }
  ...
  void onClose() {
    ...
    BleFactory.unregisterModel(RHDeviceType.powerBoard);
    // SystemChrome.setPreferredOrientations([
