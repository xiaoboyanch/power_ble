...
class OtaCtrl extends GetxController {
    ...
    powerModel.onOtaDataReceived = (value) {
      ...
      switch (value[cmdIndex]) {
        case OtaCommands.cmdQueryData_0xD0: {
          int requestType = value[cmdDataIndex + 2];
            ...
            case OtaCommands.cmdQueryData_0xD3: {
              ...
              if (state == 0) {
                ...
                if (currentPacketNum + 1 >= totalPacketCount) {
                  ...
                  //   powerModel.readRom(chip, startAddress~/ 256, startAddress% 256, length);
                  //   startAddress += 64;
                  // }
        ...
        case OtaCommands.cmdQueryData_0xD7: {
          ...
          deviceHigh = value[subCmdIndex + 2];
          deviceLow = value[subCmdIndex + 3];
          msgFlag.value++;
        }
  ...
  handshake(int chipNumber, int mode, int packetCount, int fileLength, int versionHigh, int versionLow) {
    powerModel.handshake(chipNumber, mode, packetCount, fileLength, versionHigh, versionLow);
  }

  IAPWrite(int chipNumber, int packageNum, int length, List<int> data) {
    powerModel.IAPWrite(chipNumber, packageNum, length, data);
  }

  exitBootloader() {
    powerModel.exitBootloader(chipNumber);
  }
  ...
  readRom() {
     ...
     powerModel.readRom(chip, adsH, adsL, length);
  }
  ...
  eraseRom() {
    ...
    int adsL = int.parse(adsLCtrl.text, radix: 16);
    int chip = int.parse(chipCtrl.text, radix: 16);
    powerModel.eraseRom(chip, adsH, adsL);
  }
  ...
  writeRom() {
    try {
      ...
      powerModel.writeRom(chip, adsH, adsL, length, data);
